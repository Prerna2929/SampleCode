...
@implementation WebServiceParser (Post)
                 ...
                 block:(ResponseBlock)block
{
    NSBlockOperation *operation = [NSBlockOperation blockOperationWithBlock:^{
        ...
        switch (servicetype)
        {
            ...
            case WSGetMyPost:
            {
                URLString=URLGetMyPost;
            }
                break;
            case WSCreateHashtag:
            {
                URLString=URLCreateHashTag;
            }
                break;
            case WSCreatePost:
            {
                URLString=URLCreatePost;
            }
                break;
            case WSGetAllPosts:
            {
                URLString=URLGetAllPosts;
            }
                break;
            case WSMarkAsFavorite:
            {
                URLString=URLMarkFavoritePost;
            }
                break;
            case WSGetFollowingPosts:
            {
                URLString=URLGetFollowingPosts;
            }
                break;
            case WSGetNewPost:
            {
                URLString=URLGetNewPosts;
            }
                break;
            case WSSHarePost:
            {
                URLString=URLSharePost;
            }
                break;
            default:
                break;
        ...
        if (data.length>0)
        {
            ...
            switch (servicetype)
            {
                ...
                case WSCreatePost:
                {
                    ...
                    {
                        TRC_DBG(@"Success");
                        responseString = [(NSDictionary *)Responceobjects valueForKey:@"last_post_id"];
                        //[(NSDictionary *)Responceobjects valueForKey:@"data"];
                    }
                    else if ([[(NSDictionary *)Responceobjects valueForKey:@"status"] isEqualToString:Failure])
                    {
                        NSDictionary *errorDict = [[NSDictionary alloc]initWithObjectsAndKeys:[(NSDictionary *)Responceobjects valueForKey:@"error"], @"msg", nil];
                ...
                case WSGetAllPosts:
                {
                    if ([[(NSDictionary *)Responceobjects valueForKey:@"status"] isEqualToString:Success])
                    {
                        ...
                        if ([Responceobjects isKindOfClass:[NSString class]]) {
                            Responceobjects = Responceobjects;
                        }
                        else {
                            for (NSDictionary *dict in Responceobjects) {
                                PostDetail *postDetail = [[PostDetail alloc]initWithDictionary:dict];
