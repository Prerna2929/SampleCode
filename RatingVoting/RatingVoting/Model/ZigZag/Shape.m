...
//    }
    ...
    return [[self alloc] initWithPath:path lineColor:[UIColor redColor]];
}
...
+ (UIBezierPath *)zigzag:(CGRect)bounds
{
//    CGPoint bottomLeft 	= CGPointMake(CGRectGetMinX(bounds), CGRectGetMinY(bounds));
//    CGPoint topLeft		= CGPointMake(CGRectGetMinX(bounds), CGRectGetMinY(bounds) + CGRectGetHeight(bounds) * 2.0f/3.0f);
//    CGPoint bottomRight = CGPointMake(CGRectGetMaxX(bounds), CGRectGetMinY(bounds));
...
//    CGPoint roofTip		= CGPointMake(CGRectGetMidX(bounds), CGRectGetMaxY(bounds));
    ...
    CGPoint startPoint = CGPointMake(bounds.size.width/2,0);
    
    CGPoint nextPoint;
    
    int maxY = bounds.size.height;
    ...
    for (startPoint.y = 0 ; startPoint.y <= maxY; startPoint.y+=10)
    {
        if (left)
        {
            nextPoint.y = nextPoint.y+10;
            nextPoint.x = startPoint.x-10;
        }
        else
        {
            nextPoint.x = startPoint.x+10;
            nextPoint.y = startPoint.y+10;
        }
        ...
        if (nextPoint.y >= bounds.size.height)
        {
            break;
        }
    }
